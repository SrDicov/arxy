#!/usr/bin/env bash
# test-staging.sh — staging huerfano de setup/rollback (root.new.*,
# .image.partial.*, root.old.tmp.*, .swap.*): probe via fix staging-cleanup
# (A), empates de mtime deterministas (B) y rama replace-root con R invalido
# (C). Una sola setup; sin root ni imagen (el --apply via doctor exige root:
# SKIP sin el; el apply directo via recover_staging corre en /tmp).
set -uo pipefail
FAIL=0
HERE="$(dirname "$0")"
ARXY_ROOT="/tmp/vstaging/root"
export ARXY_ROOT
# shellcheck source=lib.sh
. "$HERE/lib.sh" # ok/no/finish + arxy_mkroot + staging_clean
...
echo "== A: fix staging-cleanup detecta y resuelve =="
...
staging_clean; arxy_mkroot "$R.new.222" nuevo
fix_probe staging-cleanup FIX
...
staging_clean; arxy_mkroot "$R" bueno; arxy_mkroot "$R.old.tmp.333" previo
fix_probe staging-cleanup FIX
...
staging_clean; arxy_mkroot "$R.old.tmp.444" previo
recover_staging >/dev/null 2>&1
...
staging_clean
arxy_mkroot "$R.old.tmp.555" viejo; touch -d "2020-01-01" "$R.old.tmp.555"
arxy_mkroot "$R.old.tmp.666" reciente; touch -d "2021-01-01" "$R.old.tmp.666"
recover_staging >/dev/null 2>&1
...
staging_clean; arxy_mkroot "$R" bueno; touch "$D/.image.partial.777"
fix_probe staging-cleanup FIX
...
echo "== T7: swap varado"
staging_clean; arxy_mkroot "$R" bueno; arxy_mkroot "$R.swap.888" swapado
fix_probe staging-cleanup FIX
...
staging_clean; arxy_mkroot "$R.swap.999" swapado
recover_staging >/dev/null 2>&1
...
staging_clean; arxy_mkroot "$R" bueno; touch "$D/.image.partial.000"
if [[ "$(id -u)" -ne 0 ]]; then
...
staging_clean; arxy_mkroot "$R" bueno
...
if command -v python3 >/dev/null 2>&1; then
    staging_clean; arxy_mkroot "$R" bueno; mkdir -p "$R.new.111"; touch "$D/.image.partial.777"
...
echo "== B: dos staging validos no publican dos raices =="

echo "== TB1: empate de mtime -> uno reemplaza, el otro se borra (determinista)"
staging_clean; mkbadroot "$R"
arxy_mkroot "$R.new.AAA" aaa; arxy_mkroot "$R.new.BBB" bbb
...
echo "== TB2: recover deja root valido sin restos"
recover_staging >/dev/null 2>&1
_image_ok "$R" && ls -d "$R".new.* >/dev/null 2>&1 && no "T2 restos" || { _image_ok "$R" && ok "T2 root valido sin restos" || no "T2 root valido"; }

echo "== TB3: sin R, dos staging -> uno recupera, otro se borra"
staging_clean
arxy_mkroot "$R.new.CCC" ccc; arxy_mkroot "$R.new.DDD" ddd
...
echo "== C: R invalido + staging valido -> replace-root =="

echo "== TC1: inventory clasifica replace-root (R invalido + staging valido)"
staging_clean; mkbadroot "$R"; arxy_mkroot "$R.new.111" bueno
out="$(staging_inventory)"
grep -q "^replace-root.*root.new.111" <<<"$out" && ok "T1 replace-root" || no "T1 replace-root ($out)"

echo "== TC2: fix_probe pide reemplazar"
staging_clean; mkbadroot "$R"; arxy_mkroot "$R.new.222" bueno
fix_probe staging-cleanup FIX
[[ "${FIX[status]}" == todo && "${FIX[action]}" == *"reemplazar root con root.new.222"* ]] && ok "T2 probe" || no "T2 probe"

echo "== TC3: recover_staging deja .mark del staging, sin restos"
staging_clean; mkbadroot "$R"; arxy_mkroot "$R.new.333" bueno
recover_staging >/dev/null 2>&1
[[ "$(cat "$R/.mark" 2>/dev/null)" == bueno ]] && [[ ! -e "$R.new.333" ]] && ok "T3 reemplaza" || no "T3 reemplaza"
_image_ok "$R" && ok "T3 root valido" || no "T3 root valido"

echo "== TC4: dos staging validos -> el mas reciente reemplaza, el otro se borra"
staging_clean; mkbadroot "$R"
arxy_mkroot "$R.new.444" viejo; touch -d "2020-01-01" "$R.new.444"
arxy_mkroot "$R.new.555" reciente; touch -d "2021-01-01" "$R.new.555"
out="$(staging_inventory)"
